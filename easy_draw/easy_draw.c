...
int16_t easy_draw_height_get()
{
	return sg_screen_height;
}

int16_t easy_draw_width_get()
{
	return sg_screen_width;
}
...
#elif (ED_COLOR_DEPTH == 2)
	uint32_t buffer_size = w * h / 4;
#elif (ED_COLOR_DEPTH == 4)
	uint32_t buffer_size = w * h / 2;
#elif (ED_COLOR_DEPTH == 8)
	uint32_t buffer_size = w * h;
#elif (ED_COLOR_DEPTH == 16)
	uint32_t buffer_size = w * h * 2;
#elif (ED_COLOR_DEPTH == 24)
	uint32_t buffer_size = w * h * 3;
#endif

	uint8_t *buffer = (uint8_t *)calloc(buffer_size, 1);
	if (ED_IS_NULL(buffer))
		return NULL;

	ed_helper_buffer->width = w;
	ed_helper_buffer->height = h;
	ed_helper_buffer->data = buffer;
	ed_helper_buffer->mode = BUFFER_COVER_DEFAULT;
	ed_helper_buffer->text_format.size = 1;
	ed_helper_buffer->text_format.bk_color_isValid = 1;
	ed_helper_buffer->text_format.bk_color = 0;
...
void easy_draw_set_buffer_mode(ed_helper_buffer_t *buffer, buffer_cover_mode mode)
{
	buffer->mode = mode;
}
...
void easy_draw_buffer_negation(int16_t x, int16_t y, uint8_t w, uint8_t h)
{
	...
	for (int xi = 0; xi < w; xi++)
	{
		...
		for (int yi = 0; yi < h; yi++)
		{
			...
			uint32_t color = easy_draw_buffer_get_pixel(sx, sy, NULL);
			easy_draw_buffer_set_pixel(sx, sy, !color);
		}
	}
}
...
void easy_draw_set_text_size(uint8_t text_size)
{
	...
	if (sg_helper_buffer != NULL)
	{
		sg_helper_buffer->text_format.size = text_size;
	}
...
uint16_t easy_draw_get_text_pixels_width(void)
{
	...
	if (sg_helper_buffer != NULL)
	{
		return sg_helper_buffer->text_format.size * 6;
	}
#endif

	return 6 * sg_ed_text_config.size;
}
...
uint16_t easy_draw_get_text_pixels_hight(void)
{
	...
	if (sg_helper_buffer != NULL)
	{
		return sg_helper_buffer->text_format.size * 8;
	}
#endif
	return 8 * sg_ed_text_config.size;
}

uint16_t easy_draw_get_string_pixels_width(char *string)
{
	if (ED_IS_NULL(string))
		return 0;

#if ED_HELPER_BUFFER_DRAW_FUNC_ENABLE
	if (sg_helper_buffer != NULL)
	{
		return sg_helper_buffer->text_format.size * 6 * strlen(string);
	}
#endif
	return strlen(string) * 6 * sg_ed_text_config.size;
}
...
uint8_t easy_draw_get_text_size()
{
	...
	if (sg_helper_buffer != NULL)
	{
		return sg_helper_buffer->text_format.size;
	}
#endif
	return sg_ed_text_config.size;
}
...
ed_text_format_config_t easy_draw_get_text_format()
{
	...
	if (sg_helper_buffer != NULL)
	{
		return sg_helper_buffer->text_format;
	}
#endif
	return sg_ed_text_config;
}
...
void easy_draw_set_text_format(uint8_t text_size, uint32_t bk_color, uint8_t bk_color_isValid)
{
	...
	{
		sg_helper_buffer->text_format.size = text_size;
		sg_helper_buffer->text_format.bk_color = bk_color;
		sg_helper_buffer->text_format.bk_color_isValid = bk_color_isValid;
		return;
	}
#endif
...
void easy_draw_text_char(int16_t x, int16_t y, uint8_t c, uint32_t color)
{
	int16_t i, j;
	uint8_t a, b;
	uint8_t size = easy_draw_get_text_format().size;
	uint32_t text_color = color;
	uint32_t bk_color = easy_draw_get_text_format().bk_color;
	uint32_t bk_color_isValid = easy_draw_get_text_format().bk_color_isValid;
